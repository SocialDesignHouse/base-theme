...
function social_adrotate_to_page_options_form() { 
	...
	<form method="post">
		...
		<p>You can pull these block sections into your page templates by using the name you entered for the position in all lowercase with underscores instead of spaces as the custom post data array key.</p>
		<p>For example: (if Position was named Ad Rotate Block)<br />
			$custom = get_post_custom($post->ID);<br />
			$ad_rotate_block = $custom['ad_rotate_block'][0];
		</p>
